...
def get_stock_all(zid, start_date, end_date):
    query = f"""SELECT xitem, 
                      SUM(xqty*xsign) AS Stock,
                      EXTRACT(YEAR FROM xdate) AS year,
                      EXTRACT(MONTH FROM xdate) AS month
               FROM imtrn
               WHERE zid = {zid}
               AND xdate BETWEEN '{start_date}' AND '{end_date}'
               GROUP BY xitem, year, month order by xitem"""
...
def price(zid,start_date):
    query = f"""SELECT opddt.xitem, EXTRACT(MONTH FROM opdor.xdate), AVG(opddt.xdtwotax/opddt.xqty), AVG(opddt.xdiscf), SUM(opddt.xqty) as total_qty
                        FROM opddt
                        JOIN opdor
                        ON opddt.xdornum = opdor.xdornum
                        WHERE opdor.zid = {zid}
...
def purchase_qty(zid,start_date):
    query = f"""SELECT imtrn.xitem, imtrn.xyear, imtrn.xper ,SUM(imtrn.xqty)
                        FROM imtrn
                        WHERE imtrn.zid = {zid}
