...
#  sales from this years first date

current_date = datetime.now()

# Get the first date of the current year as a string
formatted_date = current_date.strftime('%Y-01-01')
...
# %%
file_path = f"""64_66_one_year_sale_return_stdPrice_{formatted_date}.xlsx"""
...
def get_return(zid, return_date):
    df = pd.read_sql(f"""SELECT opcrn.xcrnnum, opcrn.xdate, opcrn.xcus,cacus.xshort,cacus.xcity, opcrn.xemp, prmst.xname, opcdt.xitem, caitem.xdesc,
     caitem.xabc, sum(opcdt.xqty) as ret_qty, sum(opcdt.xlineamt) as ret_total
                            FROM opcrn
                            JOIN opcdt
...
def main (zid, end_date):
    ...
    df_stock = get_stock_all(100005,  end_date)
    ...
    df_product_price = price(100005, end_date)
...
subject = f'''H_64_66 Last One Year Sales without xval & year wise inv std price from {last_year} to {formatted_date}'''
...
#  move the excel file to all_sales folder

import shutil

shutil.move(file_path , f"""all_sales/{file_path}""")
print ("excel file moved to all_sales folder")